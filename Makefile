...
AVR_TOOLS_PATH ?= ${ARDUINO_INSTALL_DIR}/packages/arduino/tools/avr-gcc/7.3.0-atmel3.6.1-arduino7/bin/
...
BUILD_DIR          ?= $(ARDUINO_USER_DIR)/Arduino/tmp
...
#  MCU              ?= atmega2560
#  PROG_MCU         ?= m2560
#  Set to arduino, Atmega328p if not yet set.
  MCU              ?= atmega328p
  PROG_MCU         ?= m328p
...
ifeq ($(HARDWARE_VARIANT), $(filter $(HARDWARE_VARIANT),arduino Teensy Sanguino))
  ...
  VPATH += $(ARDUINO_INSTALL_DIR)/packages/arduino/hardware/avr/1.8.6/libraries/SPI/src
endif
...
ifeq ($(IS_MCU),1)
  ...
  VPATH += $(ARDUINO_INSTALL_DIR)/packages/arduino/hardware/avr/1.8.6/cores/arduino
  VPATH += $(ARDUINO_INSTALL_DIR)/packages/arduino/hardware/avr/1.8.6/libraries/SPI/src
  VPATH += $(ARDUINO_INSTALL_DIR)/packages/arduino/hardware/avr/1.8.6/libraries/SoftwareSerial/src
endif
...
LIB_CXXSRC = WMath.cpp WString.cpp Print.cpp SPI.cpp HardwareSerial.cpp HardwareSerial0.cpp HardwareSerial1.cpp HardwareSerial2.cpp HardwareSerial3.cpp
...
#$(info cxx: $(CXX))
#$(info ld_prefix: $(LD_PREFIX))
#$(info cxx_flags: $(ALL_CXXFLAGS))
#$(info obj: $(OBJ))
#$(info ld_flags: $(LDFLAGS))
#$(info ld_suffix: $(LD_SUFFIX))
...
$(BUILD_DIR)/arduino/%.o: %.c $(MAKEFILE)
	$(Pecho) "  CC    $<"
	$P $(CC) -MMD -c $(ALL_CFLAGS) $(LIBWARN) $< -o $@

$(BUILD_DIR)/arduino/%.o: %.cpp $(MAKEFILE)
	$(Pecho) "  CXX   $<"
